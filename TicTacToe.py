...
class screenGraphics(object):
    ...
    def welcomeScreen(self):
        ...
        welcomeTurtle.write("Welcome to my", move=False, align="center", font=(
            "Arial", int(BOARD_DIMENSION//10), "normal"))
        welcomeTurtle.setpos(0, BOARD_DIMENSION/3 - (BOARD_DIMENSION/9))
        welcomeTurtle.write("TicTacToe Game", move=False, align="center", font=(
            "Arial", int(BOARD_DIMENSION//10), "normal"))

        newGameButton = createButton(-BOARD_DIMENSION//9.8, -
                                     BOARD_DIMENSION//2, "New Game", "drawScreens.newGame()", 20)
    ...
    def boardScreen(self):
        ...
        for row in range(3):
            ...
            boardTurtle.up()
            boardTurtle.setpos((boardTurtle.xcor(
            ) - (BOARD_DIMENSION / 3) * 3), (boardTurtle.ycor() - (BOARD_DIMENSION / 3)))
            boardTurtle.down()

        newGameButton = createButton(-BOARD_DIMENSION//9.8+(-BOARD_DIMENSION//5), -
                                     BOARD_DIMENSION//2, "New Game", "drawScreens.newGame()", 20)
        BUTTONS.append(newGameButton)

        toWelcomeScreenButton = createButton(-BOARD_DIMENSION//9.8-(-BOARD_DIMENSION//5), -
                                             BOARD_DIMENSION//2, "Main Menu", "drawScreens.welcomeScreen()", 20)
    ...
    def endScreen(self, text_to_write):
        ...
        if text_to_write == "It's a tie":
            endTurtle.write(text_to_write, move=False, align="center", font=(
                "Arial", int(BOARD_DIMENSION//10), "normal"))
        else:
            endTurtle.write(text_to_write + " Won", move=False, align="center",
                            font=("Arial", int(BOARD_DIMENSION//10), "normal"))
...
class createButton(object):
    ...
    def text_width(self):
        ...
        self.helpTurtle.write(self.text_to_write, move=True,
                              align="left", font=("Ariel", self.font_size+3))
    ...
    def drawButton(self):
        ...
        self.helpTurtle.color("black")
        self.helpTurtle.up()
        self.helpTurtle.setpos(self.helpTurtle.xcor(
        )+(self.width)/2, self.helpTurtle.ycor() - (self.height)*0.85)
        self.helpTurtle.down()
        self.helpTurtle.write(self.text_to_write, move=True,
                              align="center", font=("Ariel", self.font_size))
...
def executeMove(row, col, selectedTurtle, text_to_write):
    ...
    selectedTurtle.up()
    selectedTurtle.goto((BOX_LOCATIONS[row][col][0]) + BOARD_DIMENSION /
                        6, BOX_LOCATIONS[row][col][1] - BOARD_DIMENSION/3)
    selectedTurtle.down()

    selectedTurtle.write(text_to_write, move=False, align="center", font=(
        "Arial", int(BOARD_DIMENSION/4), "normal"))
            ...
            for col in range(3):
                if(BOX_LOCATIONS[row][col][0] < x < BOX_LOCATIONS[row][col][0]+BOARD_DIMENSION/3) and (BOX_LOCATIONS[row][col][1]-BOARD_DIMENSION/3 < y < BOX_LOCATIONS[row][col][1]):
                    ...
                    if isvalidMove(row, col):
                        executeMove(row, col, playerTurtle,
                                    PLAYER_TEXT_TO_WRITE)
                        aiMove()
